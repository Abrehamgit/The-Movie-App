...
export class MyProvider extends Component{
    ...
    searchAhandle = (event) => {
      ...
      axios.get(`https://api.themoviedb.org/3/search
                 /movie?api_key=${ApiKey}&language=en-US&query=${this.state.searchAll}&page=1&include_adult=true`)
    ...
    render(){
       ...
       const filteredMoviesInfo = this.state.MoviesInfo.filter(movie=>(movie.title
                                                  .toLowerCase().includes(this.state.searchLatest)));
